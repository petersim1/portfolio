...
- Dealer Hits Soft 17
  - False
- Push on Dealer 22
  - False (some low minimum tables might have this...)
- Allowed to Double After Split
  - True
- Allowed to Hit After Splitting Aces
  - False
- Blackjack Payout
  - 3 / 2
- Allow Surrender
  - True
- Split Any Ten
  - True (can split 10+J, for example. Otherwise, card has to be an exact match)
...
At each state $s$, we take an action $a$, receive a reward $R$, and end up in state $s'$. Assuming that $s'$ is not a terminal state, we can evaluate the optimal $Q$ value in this new state as well (if it is terminal, we'll assume it's $0$). Essentially, we are taking a weighted average of the current $Q$ value and new information.
...
The “states” I use are: <Callout text="Player Card Total, House Card Shown, Useable Ace" />. The “actions” are: <Callout text="Hit, Stay, Double, Split, Surrender"/>. Each pair of these will have an associated Q-value. As mentioned earlier, not every action is feasible given the current state, so these unreachable states are “masked” given the valid moves determined by the Player module.
...
- $\epsilon$-Greedy policies: Each episode, with probability $\epsilon$, we randomly take an action, otherwise we take the best action. Through learning, we can decay the epsilon value, such that we transition from high exploration initially, to more exploitation later in learning.
...
I created some extensible python modules to be able to play this game. It consists of 3 main modules, which are the <Callout text="Cards, Player, Game"/> modules. I won't go into the detail of these, but we're able to efficiently dictate gameplay for 1 to many players.
...
## For each iteration i in the training loop, the lr and eps values are:
...
We can use monte-carlo methods to simulate gameplay and learn an optimal policy in each iteration. So, Q values are updated every iteration, but Q values are only evaluated every $50,000$ iterations, to speed up training.

During each evaluation period, we can aggregate average rewards, but also percent of correct moves compared to a baseline "optimal strategy" that I found online. I partition these percent correctness scores by <Callout text="Hard Total, Soft Totals, Splits"/>, so I can truly quantify where the model is performing worst. There are many intricacies that I'm leaving out, but they can all be found in the code in my github.

Some states are visited very infrequently. Empirically I found that Q values didn't converge for these. What I ended up doing is find a aggregate of cards that were visited infrequently (basically just pairs and soft totals), and I force the Player hand to be initialized with these cards every 10 iterations.

<MdImage src="/images/posts/q-learning-training-r.png">
Average rewards through training
</MdImage>

<MdImage src="/images/posts/q-learning-training-p.png">
Percent correct moves compared to the baseline optimal strategy
</MdImage>

### Evaluation

Now that the model is trained, I can evaluate its performance and its value function. Below I compare policies and their reward distributions. To do this, I play $1,000$ games of $100$ rounds of blackjack, for each policy. For each of these games, I can find the optimal average reward per round, and plot the distribution of these.

The 4 policies I evaluate are:
...
- Mimic House: stay on anything >= 17, hit otherwise

<MdImage src="/images/posts/q-learning-comparison.png" />

The average rewards, in term of units per round, are as follows:
...
- Accepted: -0.0094 net units per round. Nearly even expected value
...
Performing this for all state-action pairs, partitioned by Hard Total, Soft Total, Can Split, results in the following:

<MdImage src="/images/posts/q-learning-value-fct.png">
Value function for each state-action pair, partitioned by hand type.
</MdImage>

It's quite neat seeing these value functions arise. I think for the most part they're quite intuitive.

## Additional Analyses

There's so many additional analyses that can be done having created the Blackjack gameplay modules, outlined the blackjack environment, and trained the RL agent.
...
<MdImage src="/images/posts/q-learning-bankroll-p2.png">
Probability of being bankrolled in < N hands. All of these agents follow the same policy to generate the data.
</MdImage>

Lastly, I found it quite interesting to assess how your expected value changes according to your starting bankroll.

<MdImage src="/images/posts/q-learning-bankroll-p1.png">
As your bankroll decreases, your profitability per hand decreases as well.
</MdImage>

It's a classic example of bankroll across various games. Not matter what policy you follow, or how adept of a player you are, if you have a smaller bankroll, you are at a disadvantage. At least following a more optimal policy will help mitigate that risk, but only so much.
